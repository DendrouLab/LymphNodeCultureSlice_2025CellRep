...
#### Differential abundance analysis between control and LMQ slice lymph node
# Import data
...
slice_da1 <- colData(slice_da1)
diff_abundance_s1 <- as.data.frame(table(slice_da1$sample_id, slice_da1$celltypes))
colnames(diff_abundance_s1) <- c("sample_id", "cell_type", "number_of_cells")
diff_abundance_s1 <- separate(data = diff_abundance_s1, col = "sample_id", c('donor_id', 'sample_type'), sep = "_", remove = FALSE)

diff_abundance_s1$donor_id <- as.factor(diff_abundance_s1$donor_id)
diff_abundance_s1$sample_id <- as.factor(diff_abundance_s1$sample_id)
diff_abundance_s1$sample_type <- as.factor(diff_abundance_s1$sample_type)

# Adding in a column with the total number of cells per sample
cells_per_sample1 <- data.frame(table(slice_da1$sample_id))
colnames(cells_per_sample1) <- c("sample_id", "cells_per_sample1")
matching_rows1 <- match(diff_abundance_s1$sample_id, cells_per_sample1$sample_id)
diff_abundance_s1$cells_per_sample1 <- cells_per_sample1$cells_per_sample1[matching_rows1]

# Calculating the proportion of each cell type per sample
diff_abundance_s1$percentage_cells <- diff_abundance_s1$number_of_cells/diff_abundance_s1$cells_per_sample1

# Quantifying the significance by performing an anova test where the overall error sum of squares and degrees of freedom are calculated from a linear model
cd19.model <- lm(percentage_cells ~ cell_type + sample_type, data = diff_abundance_s1)
temp <- diff_abundance_s1 %>%
  group_by(cell_type) %>%
  anova_test(percentage_cells ~ sample_type, error = cd19.model)

# Remove NA percentage results
filtered_diff_abundance1 <- diff_abundance_s1 %>% filter(!is.na(percentage_cells))

filtered_diff_abundance1 <- filtered_diff_abundance1 %>%
...
ggplot(filtered_diff_abundance1, aes(x = factor(cell_type, level = c("Stromal", "Endothelial", "CD16hi_NK", "CD16low_NK", "SELL+ID2hi_ILC", "ILC3", "HLADRhi_ILC3", "IL2RA+IL22+_ILC3", "IL2RA+CXCL8+_ILC", "HSPhi_ILC", "DC", "pDC", "Monocyte_Macrophages", "Mast_cells", "Proliferating_cells")), 
  ...
  geom_point(aes(color = donor_id, group = combined_type), size = 1, position = position_dodge(width = 0.75), col = "black") +
  labs(x = "Cell Type", y = "Proportion of cells (average across donors)", fill = "Sample Type") +
  theme(text = element_text(size = 15), 
        axis.text.x = element_text(angle = 55, hjust = 1),
        panel.background = element_rect(fill = "white", colour = "darkgray", size = 2),
        panel.grid.major = element_line(colour = "gray", size = 0.3)) +
  scale_fill_manual(values = c("#E75480", "#dcbeff")) +
  scale_color_discrete(guide = guide_legend(title = "Donor ID")) 
